...
def get_settings(workspace_id: UUID = Depends(get_workspace_id), current_user: User = Depends(require_min_role(RoleName.OWNER)), db: Session = Depends(get_db)) -> NovaPoshtaSettingsResponse:
...
def test_connection(workspace_id: UUID = Depends(get_workspace_id), current_user: User = Depends(require_min_role(RoleName.OWNER)), db: Session = Depends(get_db)) -> NovaPoshtaTestConnectionResponse:
    try:
...
def disconnect(workspace_id: UUID = Depends(get_workspace_id), current_user: User = Depends(require_min_role(RoleName.OWNER)), db: Session = Depends(get_db)) -> NovaPoshtaSettingsResponse:
...
def search_cities(q: str = Query(min_length=2), limit: int = Query(default=20, ge=1, le=50), workspace_id: UUID = Depends(get_workspace_id), current_user: User = Depends(require_min_role(RoleName.MANAGER)), db: Session = Depends(get_db)) -> list[NovaPoshtaDirectoryItem]:
    try:
...
def search_warehouses(city_ref: str = Query(min_length=1), q: str | None = Query(default=None), limit: int = Query(default=50, ge=1, le=100), workspace_id: UUID = Depends(get_workspace_id), current_user: User = Depends(require_min_role(RoleName.MANAGER)), db: Session = Depends(get_db)) -> list[NovaPoshtaDirectoryItem]:
    try:
