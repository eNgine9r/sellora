...
class InstagramOAuthService:
    ...
    def start(self, workspace_id: UUID, user_id: UUID) -> tuple[str, datetime]:
        settings = get_settings()
        if not settings.meta_app_id or not settings.meta_oauth_redirect_uri:
            raise MetaInstagramError("META_CONFIGURATION_MISSING", "Instagram OAuth is not configured.")
    ...
    def validate_state(self, raw_state: str, workspace_id: UUID, user_id: UUID) -> MetaOAuthState:
        item = MetaOAuthStateRepository(self.db).get_by_hash_for_update(hashlib.sha256(raw_state.encode()).hexdigest())
        if not item or item.workspace_id != workspace_id or item.user_id != user_id:
            raise MetaInstagramError("META_OAUTH_STATE_INVALID", "OAuth state is invalid.")
        if item.consumed_at:
            raise MetaInstagramError("META_OAUTH_STATE_ALREADY_USED", "OAuth state was already used.")
        if item.expires_at < datetime.now(UTC):
            raise MetaInstagramError("META_OAUTH_STATE_EXPIRED", "OAuth state expired.")
        return item
