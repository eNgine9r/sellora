...
class InstagramConnectionRepository:
    ...
    def get_active(self, workspace_id: UUID) -> InstagramConnection | None:
        return self.db.execute(
            ...
            .where(
                InstagramConnection.workspace_id == workspace_id,
                InstagramConnection.deleted_at.is_(None),
            )
            .order_by(InstagramConnection.created_at.desc(), InstagramConnection.id.desc())
            .limit(1)
        ).scalar_one_or_none()
    ...
    def get_by_account(self, instagram_account_id: str) -> InstagramConnection | None:
        rows = self.db.execute(
            select(InstagramConnection)
            .where(
                InstagramConnection.instagram_account_id == instagram_account_id,
                InstagramConnection.status == InstagramConnectionStatus.CONNECTED.value,
                InstagramConnection.deleted_at.is_(None),
            )
            .order_by(InstagramConnection.updated_at.desc(), InstagramConnection.id.desc())
            .limit(2)
        ).scalars().all()
        if len(rows) != 1:
            return None
    ...
    def get_connected_in_other_workspace(self, instagram_account_id: str, workspace_id: UUID) -> InstagramConnection | None:
        return self.db.execute(
            select(InstagramConnection)
            .where(
                InstagramConnection.instagram_account_id == instagram_account_id,
...
class MetaOAuthStateRepository:
    ...
    def get_by_hash_for_update(self, state_hash: str) -> MetaOAuthState | None:
        return self.db.execute(
            select(MetaOAuthState).where(MetaOAuthState.state_hash == state_hash).with_for_update()
        ).scalar_one_or_none()
