...
Sprint 8E adds server-side provider-write gating, duplicate TTN protection, deterministic status mapping documentation, and controlled-pilot guidance for Nova Poshta. Real provider TTN approval remains blocked until approved staging secrets and guarded `STAGING_NOVA_POSHTA_ALLOW_WRITES=true` workflow evidence are available.
