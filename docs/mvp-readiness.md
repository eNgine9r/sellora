...
Nova Poshta provider writes now require an explicit server-side staging flag and TTN creation has duplicate/in-progress protection. Real provider validation remains blocked until staging secrets, controlled TTN creation, browser/mobile QA, and cleanup evidence are available.
