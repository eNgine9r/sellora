...
## Sprint 4.4 manual attribution limitations

Manual attribution is optional and workspace-scoped. Sellora does not automatically match Meta clicks, Instagram Direct messages, leads, customers, or orders. Orders without a campaign are valid and appear as unattributed. The attribution panel counts only orders where a campaign was selected manually.

Future Meta attribution may use external campaign/ad identifiers, but Meta Ads OAuth, automatic Meta Ads sync, and automatic attribution are out of scope. Advertising import remains not pilot-ready until deployed manual staging QA passes. Sprint 4.3 full approval remains blocked where frontend lockfile/build/browser QA cannot be recovered.
