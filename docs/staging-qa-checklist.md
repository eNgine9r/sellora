...
- [ ] Create a lead without a campaign and confirm the empty state says the campaign is not set.
- [ ] Create a lead with a campaign selected by human-readable campaign name.
- [ ] Create an order without a campaign and confirm it remains valid.
- [ ] Create an order with a campaign selected by human-readable campaign name.
- [ ] Edit an order to add, change, and remove the campaign.
- [ ] Confirm `/advertising` shows Manual attribution MVP with campaign-linked orders, attributed revenue, attributed net profit, and unattributed orders.
- [ ] Confirm manual attribution is workspace-scoped and cross-workspace campaign links are rejected by backend validation.
- [ ] Confirm future Meta attribution is described as future work only.
- [ ] Confirm advertising import is not pilot-ready until deployed manual import QA passes.
- [ ] Confirm Sprint 4.3 build/browser approval remains blocked if frontend lockfile/dependency installation is unavailable.
