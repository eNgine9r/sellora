...
export function ImportReportPanel({ report }: { report?: ImportReport }) {
  ...
  return <section className="w-full min-w-0 max-w-full overflow-hidden rounded-xl bg-white p-4 shadow-sm"><h2 className="font-semibold">Dry run report</h2><div className="mt-3 grid min-w-0 gap-2 text-sm sm:grid-cols-2 md:grid-cols-4"><span>Total {report.total_rows}</span><span>Ready {report.ready_to_import_rows}</span><span>Warnings {report.warning_rows}</span><span>Errors {report.error_rows}</span><span>Duplicates {report.duplicate_rows}</span><span>Skipped {report.skipped_rows}</span><span>Estimate {report.estimated_entities_to_create}</span></div>{catalogMetrics.length ? <MetricBlock title="Product catalog counters" tone="blue" metrics={catalogMetrics} /> : null}{orderMetrics.length ? <MetricBlock title="Historical orders counters" tone="amber" metrics={orderMetrics} /> : null}{adMetrics.length ? <MetricBlock title="Historical advertising counters" tone="purple" metrics={adMetrics} /> : null}<div className="mt-4 grid min-w-0 gap-4 md:grid-cols-2"><div><h3 className="font-medium text-red-600">Sample errors</h3>{report.sample_errors.map((item, index) => <p className="break-words text-sm" key={index}>Row {item.row_number}: {item.message}</p>)}</div><div><h3 className="font-medium text-amber-600">Sample warnings</h3>{report.sample_warnings.map((item, index) => <p className="break-words text-sm" key={index}>Row {item.row_number}: {item.message}</p>)}</div></div></section>;
}
