...
export function ImportUploadCard({ onUpload, isUploading = false }: { onUpload: (file: File) => void; isUploading?: boolean }) {
  const { t } = useI18n();
  ...
  return (
    <section className="sellora-card w-full min-w-0 max-w-full overflow-hidden rounded-xl bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 grid w-full min-w-0 max-w-full gap-2 sm:grid-cols-[auto_minmax(0,1fr)] sm:items-center">
        ...
        <button className="min-h-11 w-full min-w-0 max-w-full rounded-xl bg-blue-600 px-4 py-2 font-bold text-white sm:w-auto" type="button" onClick={() => inputRef.current?.click()} disabled={isUploading}>{isUploading ? t("importCenter.uploading") : t("importCenter.chooseFile")}</button>
      ...
      <p className="mt-2 break-words text-sm text-slate-500">{t("importCenter.uploadLimits")}</p>
    </section>
  );
}
