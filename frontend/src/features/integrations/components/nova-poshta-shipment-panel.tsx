...
export function NovaPoshtaShipmentPanel({ workspaceId, shipment }: { workspaceId: string; shipment: Shipment }) {
  ...
  return (
    <section className="grid gap-3 rounded-xl bg-slate-50 p-3 text-sm dark:bg-white/[0.04]">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="font-bold text-slate-950 dark:text-white">{t("shipments.novaPoshtaSection")}</h3>
        <span className={`rounded-full px-3 py-1 text-xs font-bold ${hasTtn ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-100" : "bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-100"}`}>{hasTtn ? t("novaPoshta.ttnExists") : t("shipments.ttnMissing")}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <span className="text-slate-500 dark:text-slate-400">{t("novaPoshta.cityRef")}</span><strong className="break-all">{shipment.nova_poshta_city_ref ?? "—"}</strong>
        <span className="text-slate-500 dark:text-slate-400">{t("novaPoshta.warehouseRef")}</span><strong className="break-all">{shipment.nova_poshta_warehouse_ref ?? "—"}</strong>
        <span className="text-slate-500 dark:text-slate-400">{t("novaPoshta.documentRef")}</span><strong className="break-all">{shipment.nova_poshta_document_ref ?? "—"}</strong>
      ...
      <CreateTtnButton workspaceId={workspaceId} shipmentId={shipment.id} hasTtn={hasTtn} />
