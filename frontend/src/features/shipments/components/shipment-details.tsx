...
export function ShipmentDetails({ shipment, onStatusChange }: { shipment: Shipment; onStatusChange: (status: ShipmentStatus) => void }) {
  const actions = NEXT_ACTIONS[shipment.status];
  return (
    <aside className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm"><span className="text-slate-500">Order</span><strong>{shipment.order_number ?? "—"}</strong><span className="text-slate-500">Customer</span><strong>{shipment.customer_name ?? "—"}</strong><span className="text-slate-500">Carrier</span><strong>{shipment.carrier}</strong><span className="text-slate-500">City</span><strong>{shipment.city ?? "—"}</strong><span className="text-slate-500">Warehouse</span><strong>{shipment.warehouse ?? "—"}</strong><span className="text-slate-500">Shipping</span><strong>{shipment.shipping_cost ?? "—"}</strong><span className="text-slate-500">COD</span><strong>{shipment.cod_amount ?? "—"}</strong></div>
      ...
      {shipment.notes ? <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-600">{shipment.notes}</p> : null}
