...
export function CampaignForm({ onSubmit }: { onSubmit?: (payload: AdCampaignCreate) => void }) {
  ...
  return (
    <form className="grid w-full min-w-0 max-w-full gap-3 overflow-hidden rounded-2xl bg-white p-4 shadow-sm" onSubmit={submit}>
      <h2 className="text-lg font-semibold">{t("advertising.createCampaign")}</h2>
      <input className="w-full min-w-0 max-w-full rounded-md border border-slate-300 px-3 py-2" placeholder={t("advertising.campaignName")} required value={values.name ?? ""} onChange={(event) => setValues({ ...values, name: event.target.value })} />
      ...
      <div className="grid min-w-0 gap-3 sm:grid-cols-2">
        <input className="w-full min-w-0 max-w-full rounded-md border border-slate-300 px-3 py-2" inputMode="decimal" placeholder={t("advertising.dailyBudget")} value={values.daily_budget ?? ""} onChange={(event) => setValues({ ...values, daily_budget: event.target.value })} />
        <input className="w-full min-w-0 max-w-full rounded-md border border-slate-300 px-3 py-2" inputMode="decimal" placeholder={t("advertising.totalBudget")} value={values.total_budget ?? ""} onChange={(event) => setValues({ ...values, total_budget: event.target.value })} />
      ...
      <textarea className="w-full min-w-0 max-w-full rounded-md border border-slate-300 px-3 py-2" placeholder={t("advertising.notes")} value={values.notes ?? ""} onChange={(event) => setValues({ ...values, notes: event.target.value })} />
      {validationError ? <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700">{validationError}</p> : null}
      <button className="min-h-11 w-full min-w-0 max-w-full rounded-md bg-blue-600 px-4 py-2 font-semibold text-white" type="submit">{t("advertising.createCampaign")}</button>
    </form>
  );
}
