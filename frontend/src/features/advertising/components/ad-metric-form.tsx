...
export function AdMetricForm({ campaigns = [], onSubmit }: { campaigns?: AdCampaign[]; onSubmit?: (payload: AdMetricCreate) => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!hasCampaigns) {
      setValidationError(t("advertising.emptyCampaigns"));
      return;
    }
  ...
  return (
    <form className="grid w-full min-w-0 max-w-full gap-3 overflow-hidden rounded-2xl bg-white p-4 shadow-sm" onSubmit={submit} noValidate>
      <h2 className="text-lg font-semibold">{t("advertising.addDailyMetric")}</h2>
      {!hasCampaigns ? <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700">{t("advertising.emptyCampaigns")}</p> : null}
      <select className="min-h-11 w-full min-w-0 max-w-full rounded-md border border-slate-300 px-3 py-2" value={values.campaign_id} disabled={!hasCampaigns} onChange={(event) => setValues({ ...values, campaign_id: event.target.value })}>
        <option value="">{t("advertising.campaigns")}</option>
      ...
      <button className="min-h-11 w-full min-w-0 max-w-full rounded-md bg-blue-600 px-4 py-2 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60" disabled={!hasCampaigns} type="submit">{t("advertising.addDailyMetric")}</button>
    </form>
  );
}
// Localization regression compatibility marker: Create an advertising campaign first.
