...
export function OrderForm({ variants, onSubmit }: { variants: ProductVariant[]; onSubmit: (values: OrderCreatePayload) => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!hasVariants) {
      setValidationError("Create a product variant first before creating an order.");
      return;
    }
  ...
  return (
    <form className="grid gap-3" onSubmit={submit} noValidate>
      ...
      <select className="min-h-11 rounded-md border border-slate-300 px-3 py-2" value={item.product_variant_id} disabled={!hasVariants} onChange={(event) => setValues({ ...values, items: [{ ...item, product_variant_id: event.target.value }] })}><option value="">Select variant</option>{variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.sku}</option>)}</select>
