  ...
  return (
    <main className="min-h-screen bg-slate-100 p-4 sm:p-6 text-slate-950">
      <div className="mx-auto grid min-w-0 max-w-7xl gap-6">
        ...
        {isCreateOpen ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/40 p-3 sm:p-4"><div className="w-full max-w-3xl overflow-hidden rounded-2xl bg-white p-4 shadow-xl sm:p-6"><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-bold">Create order</h2><button className="text-slate-500" onClick={() => setIsCreateOpen(false)}>Close</button></div><OrderForm variants={variantsQuery.data ?? []} products={productsQuery.data ?? []} inventory={inventoryQuery.data ?? []} currencyCode={currencyCode} showProfit={currentWorkspace?.role === "OWNER"} onSubmit={(values) => createMutation.mutate(values as Parameters<typeof createOrder>[1])} /></div></div> : null}
        {editingOrder ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/40 p-3 sm:p-4"><div className="w-full max-w-3xl overflow-hidden rounded-2xl bg-white p-4 shadow-xl sm:p-6"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-bold">Edit order</h2><p className="text-sm text-slate-500">Order items can be edited while the order is NEW or CONFIRMED.</p></div><button className="text-slate-500" onClick={() => setEditingOrder(null)}>Close</button></div><OrderForm variants={variantsQuery.data ?? []} products={productsQuery.data ?? []} inventory={inventoryQuery.data ?? []} currencyCode={currencyCode} initialOrder={editingOrder} lockedItems={!ITEM_EDIT_STATUSES.includes(editingOrder.status)} submitLabel="Save order" showProfit={currentWorkspace?.role === "OWNER"} onSubmit={(values) => updateMutation.mutate(values as Parameters<typeof updateOrder>[2])} />{updateMutation.isError ? <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700">{safeApiErrorMessage(updateMutation.error, "Please check the order items and costs.")}</p> : null}</div></div> : null}
