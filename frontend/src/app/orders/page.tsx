...
export default function OrdersPage() {
  ...
  const createMutation = useMutation({ mutationFn: (values: Parameters<typeof createOrder>[1]) => createOrder(workspaceId, values), onSuccess: () => { setIsCreateOpen(false); queryClient.invalidateQueries({ queryKey: ["orders", workspaceId] }); queryClient.invalidateQueries({ queryKey: ["orders-dashboard", workspaceId] }); } });
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-4 sm:p-6 text-slate-950">
      <div className="mx-auto grid max-w-7xl gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]"><OrderTable orders={ordersQuery.data ?? []} onSelect={setSelectedOrder} />{selectedOrder ? <OrderDetails order={selectedOrder} shipment={shipmentQuery.data} onStatusChange={(nextStatus) => statusMutation.mutate({ orderId: selectedOrder.id, nextStatus })} /> : <div className="rounded-xl border border-slate-200 bg-white p-4 text-slate-500">Select an order to view details, status history, and profit.</div>}</div>
