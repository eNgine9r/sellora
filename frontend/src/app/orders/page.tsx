...
export default function OrdersPage() {
  ...
  const ordersQuery = useQuery({ queryKey: ["orders", workspaceId, status], queryFn: () => fetchOrders(workspaceId, status, token), enabled });
  const dashboardQuery = useQuery({ queryKey: ["orders-dashboard", workspaceId], queryFn: () => fetchOrderDashboard(workspaceId, token), enabled });
  const variantsQuery = useQuery({ queryKey: ["product-variants", workspaceId], queryFn: () => fetchProductVariants(workspaceId, undefined, token), enabled });
  const createMutation = useMutation({ mutationFn: (values: Parameters<typeof createOrder>[1]) => createOrder(workspaceId, values, token), onSuccess: () => { setIsCreateOpen(false); queryClient.invalidateQueries({ queryKey: ["orders", workspaceId] }); queryClient.invalidateQueries({ queryKey: ["orders-dashboard", workspaceId] }); } });
  const statusMutation = useMutation({ mutationFn: ({ orderId, nextStatus }: { orderId: string; nextStatus: OrderStatus }) => changeOrderStatus(workspaceId, orderId, nextStatus, token), onSuccess: (order) => { setSelectedOrder(order); queryClient.invalidateQueries({ queryKey: ["orders", workspaceId] }); queryClient.invalidateQueries({ queryKey: ["orders-dashboard", workspaceId] }); } });
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-6 text-slate-950">
      <div className="mx-auto grid max-w-7xl gap-6">
        ...
        <section className="grid gap-3 rounded-2xl bg-white p-4 shadow-sm md:grid-cols-4"><input className="rounded-md border border-slate-300 px-3 py-2" placeholder="Workspace ID" value={workspaceId} onChange={(event) => setWorkspaceId(event.target.value)} /><input className="rounded-md border border-slate-300 px-3 py-2" placeholder="Access token" value={token} onChange={(event) => setToken(event.target.value)} /><select className="rounded-md border border-slate-300 px-3 py-2" value={status} onChange={(event) => setStatus(event.target.value as OrderStatus | "")}>{STATUSES.map((item) => <option key={item || "all"} value={item}>{item || "All statuses"}</option>)}</select><div className="text-sm text-slate-600">Today: {dashboardQuery.data?.orders_today ?? 0} orders · ${dashboardQuery.data?.revenue_today ?? "0"} revenue · ${dashboardQuery.data?.profit_today ?? "0"} profit</div></section>
