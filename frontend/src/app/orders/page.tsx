  ...
  return (
    <main className="min-h-screen min-w-0 overflow-x-hidden bg-slate-100 p-4 text-slate-950 sm:p-6">
      <div className="mx-auto grid min-w-0 max-w-7xl gap-6">
        ...
        {isCreateOpen ? <FormDialog title="Create order" description="Reserve inventory, add costs, and keep profit totals in the same mobile-safe modal style." size="xl" onClose={() => setIsCreateOpen(false)}><OrderForm variants={variantsQuery.data ?? []} products={productsQuery.data ?? []} inventory={inventoryQuery.data ?? []} currencyCode={currencyCode} showProfit={currentWorkspace?.role === "OWNER"} onSubmit={(values) => createMutation.mutate(values as Parameters<typeof createOrder>[1])} /></FormDialog> : null}
        {editingOrder ? <FormDialog title="Edit order" description="Order items can be edited while the order is NEW or CONFIRMED." size="xl" onClose={() => setEditingOrder(null)}><OrderForm variants={variantsQuery.data ?? []} products={productsQuery.data ?? []} inventory={inventoryQuery.data ?? []} currencyCode={currencyCode} initialOrder={editingOrder} lockedItems={!ITEM_EDIT_STATUSES.includes(editingOrder.status)} submitLabel="Save order" showProfit={currentWorkspace?.role === "OWNER"} onSubmit={(values) => updateMutation.mutate(values as Parameters<typeof updateOrder>[2])} />{updateMutation.isError ? <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700 dark:bg-amber-500/15 dark:text-amber-100">{safeApiErrorMessage(updateMutation.error, "Please check the order items and costs.")}</p> : null}</FormDialog> : null}
