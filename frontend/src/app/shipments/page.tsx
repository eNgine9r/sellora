...
export default function ShipmentsPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-4 text-slate-950 sm:p-6">
      ...
      {archivingShipment ? <ConfirmActionDialog title="Archive shipment?" description={archivingShipment.nova_poshta_document_ref ? "This only archives the Sellora shipment record. It does not cancel the Nova Poshta TTN." : "This shipment will be hidden from active logistics lists. The related order remains unchanged."} actionLabel={t("shipments.archive")} isSubmitting={archiveMutation.isPending} error={archiveMutation.isError ? safeApiErrorMessage(archiveMutation.error, "Unable to delete record. Please try again.") : null} onCancel={() => setArchivingShipment(null)} onConfirm={() => archiveMutation.mutate()} /> : null}
      ...
      {isCreateOpen ? <FormDialog title={t("shipments.create")} description="Manual shipment entry remains available; Nova Poshta fields are optional until TTN creation." size="xl" onClose={() => setIsCreateOpen(false)}><ShipmentForm orders={ordersQuery.data ?? []} workspaceId={workspaceId} onSubmit={(payload) => createMutation.mutate(payload)} /></FormDialog> : null}
    </main>
  );
}
