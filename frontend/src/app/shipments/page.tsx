...
export default function ShipmentsPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-4 text-slate-950 sm:p-6">
      ...
      {isCreateOpen ? <FormDialog title="Create shipment" description="Manual shipment entry remains available; Nova Poshta fields are optional until TTN creation." size="xl" onClose={() => setIsCreateOpen(false)}><ShipmentForm orders={ordersQuery.data ?? []} workspaceId={workspaceId} onSubmit={(payload) => createMutation.mutate(payload)} /></FormDialog> : null}
    </main>
  );
}
