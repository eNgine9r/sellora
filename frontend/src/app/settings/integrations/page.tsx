...
export default function IntegrationsSettingsPage() {
  ...
  const test = useMutation({ mutationFn: () => testNovaPoshtaConnection(workspaceId), onSuccess: (result) => { setMessage(result.message); queryClient.invalidateQueries({ queryKey: ["np-settings", workspaceId] }); }, onError: (error) => setMessage(safeApiErrorMessage(error, t("novaPoshta.testFailed"))) });
