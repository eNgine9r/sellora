...
export default function AnalyticsPage() {
  ...
  return <main className="min-h-screen min-w-0 overflow-x-hidden bg-[#F8F7FC] p-4 sm:p-6 text-slate-950"><div className="mx-auto grid min-w-0 max-w-7xl gap-6"><header className="min-w-0 rounded-2xl bg-white p-6 shadow-sm"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-600">Sellora</p><h1 className="mt-2 text-3xl font-bold">{t("analytics.title")}</h1><p className="mt-1 text-slate-600">{t("analytics.subtitle")}</p></header><DateRangeFilter startDate={startDate} endDate={endDate} onStartDateChange={setStartDate} onEndDateChange={setEndDate} /><section><h2 className="mb-3 text-xl font-bold">{t("dashboard.salesChart")}</h2><div className="grid min-w-0 gap-4 md:grid-cols-4"><AnalyticsKpiCard label="Total Orders" value={sales.data?.total_orders ?? 0} /><AnalyticsKpiCard label={t("dashboard.revenue")} value={formatMoney(sales.data?.total_revenue, currencyCode)} /><AnalyticsKpiCard label="AOV" value={formatMoney(sales.data?.average_order_value, currencyCode)} /><AnalyticsKpiCard label="Completed" value={sales.data?.completed_orders ?? 0} /></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><RevenueTrendChart data={trend.data ?? []} /><OrdersTrendChart data={trend.data ?? []} /></div></section><section><h2 className="mb-3 text-xl font-bold">{t("tables.profit")}</h2><div className="grid min-w-0 gap-4 md:grid-cols-4"><AnalyticsKpiCard label={t("dashboard.netProfit")} value={profit.data?.total_net_profit != null ? formatMoney(profit.data.total_net_profit, currencyCode) : "Restricted"} /><AnalyticsKpiCard label="Margin %" value={profit.data?.margin_percent ?? "Restricted"} /><AnalyticsKpiCard label="Product Cost" value={profit.data?.total_product_cost != null ? formatMoney(profit.data.total_product_cost, currencyCode) : "Restricted"} /><AnalyticsKpiCard label="Other Costs" value={profit.data?.total_other_cost != null ? formatMoney(profit.data.total_other_cost, currencyCode) : "Restricted"} /></div><div className="mt-4"><ProfitTrendChart data={trend.data ?? []} /></div></section><section><h2 className="mb-3 text-xl font-bold">{t("navigation.products")}</h2><TopProductsTable products={products.data ?? []} /></section><section><h2 className="mb-3 text-xl font-bold">{t("navigation.customers")}</h2><div className="mb-4 grid gap-4 md:grid-cols-4"><AnalyticsKpiCard label="Total Customers" value={customers.data?.total_customers ?? 0} /><AnalyticsKpiCard label="New Customers" value={customers.data?.new_customers ?? 0} /><AnalyticsKpiCard label="Repeat Customers" value={customers.data?.repeat_customers ?? 0} /><AnalyticsKpiCard label="Repeat Rate" value={customers.data?.repeat_purchase_rate ?? "0.00"} /></div><TopCustomersTable customers={customers.data?.top_customers ?? []} /></section><section><h2 className="mb-3 text-xl font-bold">{t("navigation.inventory")}</h2><div className="mb-4 grid gap-4 md:grid-cols-4"><AnalyticsKpiCard label="Total Variants" value={inventory.data?.total_variants ?? 0} /><AnalyticsKpiCard label={t("inventory.lowStock")} value={inventory.data?.low_stock_count ?? 0} /><AnalyticsKpiCard label="Out of Stock" value={inventory.data?.out_of_stock_count ?? 0} /><AnalyticsKpiCard label="Stock Units" value={inventory.data?.total_stock_units ?? 0} /></div><LowStockTable items={inventory.data?.low_stock_items ?? []} /></section></div></main>;
}
