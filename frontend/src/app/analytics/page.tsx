      ...
      <div className="mx-auto grid min-w-0 max-w-7xl gap-6">
        ...
        <ReportCard title={t("analytics.insights.title")} subtitle={t("analytics.insights.subtitle")}>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {insights.map((insight) => <Link key={`${insight.sourceMetric}-${insight.titleKey}`} href={insight.href ?? "/analytics"} className="rounded-2xl border border-slate-100 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-violet-200 hover:bg-violet-50 dark:border-white/10 dark:bg-white/[0.05] dark:hover:bg-violet-400/10"><span className="text-xs font-black uppercase tracking-[0.18em] text-violet-600 dark:text-violet-200">{t(`analytics.insights.types.${insight.type}`)} · {insight.sourceMetric}</span><h3 className="mt-2 font-black text-slate-950 dark:text-white">{t(insight.titleKey, insight.values)}</h3><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{t(insight.descriptionKey, insight.values)}</p>{insight.ctaKey ? <p className="mt-3 text-sm font-bold text-violet-700 dark:text-violet-200">{t(insight.ctaKey)}</p> : null}</Link>)}
          </div>
        </ReportCard>

        <ReportCard title={t("analytics.sales.title")} subtitle={t("analytics.sales.subtitle")}>
          <div className="mb-4 grid gap-3 md:grid-cols-4"><MetricCard label={t("analytics.metrics.revenue")} value={formatMoney(orderSummary.revenue, currencyCode)} helper={t("analytics.tooltips.revenue")} /><MetricCard label={t("analytics.metrics.netProfit")} value={canSeeProfit ? formatMoney(profit.data?.total_net_profit ?? orderSummary.netProfit, currencyCode) : t("analytics.metrics.restricted")} helper={t("analytics.tooltips.netProfit")} /><MetricCard label={t("analytics.metrics.aov")} value={orderSummary.aov == null ? UNAVAILABLE : formatMoney(orderSummary.aov, currencyCode)} helper={t("analytics.tooltips.aov")} /><MetricCard label={t("analytics.metrics.returnRate")} value={formatPercentValue(orderSummary.returnRate)} helper={t("analytics.tooltips.returnRate")} /></div>
        ...
        <ReportCard title={t("analytics.advertising.title")} subtitle={t("analytics.advertising.subtitle")}>
          <div className="mb-4 grid gap-3 md:grid-cols-4"><MetricCard label={t("analytics.metrics.spend")} value={formatMoney(adSummary.spend, currencyCode)} /><MetricCard label={t("analytics.metrics.roas")} value={formatDecimal(adSummary.roas)} helper={t("analytics.tooltips.roas")} /><MetricCard label={t("analytics.metrics.cpa")} value={adSummary.cpa == null ? UNAVAILABLE : formatMoney(adSummary.cpa, currencyCode)} helper={t("analytics.tooltips.cpa")} /><MetricCard label={t("analytics.metrics.cpl")} value={adSummary.cpl == null ? UNAVAILABLE : formatMoney(adSummary.cpl, currencyCode)} helper={t("analytics.tooltips.cpl")} /></div>
