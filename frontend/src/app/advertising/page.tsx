...
const PLATFORM_OPTIONS = [["META", "Meta"], ["INSTAGRAM", "Instagram"], ["FACEBOOK", "Facebook"], ["TIKTOK", "TikTok"], ["GOOGLE", "Google"], ["TELEGRAM", "Telegram"], ["OTHER", "Інше"]].map(([value, label]) => ({ value, label }));
const STATUS_OPTIONS = [["ACTIVE", "Активна"], ["PAUSED", "Призупинена"], ["COMPLETED", "Завершена"], ["ARCHIVED", "Архівна"]].map(([value, label]) => ({ value, label }));
const OBJECTIVE_OPTIONS = [["MESSAGES", "Повідомлення"], ["SALES", "Продажі"], ["TRAFFIC", "Трафік"], ["AWARENESS", "Впізнаваність"], ["FOLLOWERS", "Підписники"], ["OTHER", "Інше"]].map(([value, label]) => ({ value, label }));
const BUDGET_OPTIONS = [["DAILY", "Денний"], ["LIFETIME", "На весь період"], ["MANUAL", "Ручний"]].map(([value, label]) => ({ value, label }));
...
function StatusList({ title, items, tone }: { title: string; items: string[]; tone: "ready" | "pending" | "future" }) {
  ...
  return <div className={`rounded-2xl border p-4 text-sm ${toneClasses[tone]}`}><p className="font-black">{title}</p><ul className="mt-2 grid gap-1 pl-4 list-disc">{items.map((item) => <li key={item}>{item}</li>)}</ul></div>;
}
...
export default function AdvertisingPage() {
  ...
  const readinessReady = [t("advertising.readinessReadyManualMetrics"), t("advertising.readinessReadyCsvTemplate"), t("advertising.readinessReadyFormulas"), t("advertising.readinessReadyInsights"), t("advertising.readinessReadyManualAttribution")];
  const readinessPending = [t("advertising.readinessPendingImportQa"), t("advertising.readinessPendingPostgresMigration"), t("advertising.readinessPendingBrowserMobileQa")];
  const invalidateAds = () => {
  ...
  return (
    <WorkspacePage>
      ...
      <section className="grid gap-2 rounded-2xl border border-warning/20 bg-warning/10 p-4 text-sm text-warning-foreground" data-advertising-reporting-source="manual-import-meta-future"><p className="font-bold">{t("advertising.dataSourceTitle")}: {t("advertising.manualSource")}</p><p>{t("advertising.manualImportFirst")}</p><p>{t("advertising.futureMetaSync")}</p><p>{t("advertising.formulaSafety")}</p></section>
      ...
      <section className="grid w-full max-w-full min-w-0 gap-3 overflow-hidden rounded-2xl bg-white p-4 shadow-sm md:grid-cols-2 xl:grid-cols-4" data-advertising-reporting-polish="manual-import-attribution-optional">
        <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-950"><p className="font-bold">{t("advertising.importMetricsFirst")}</p><p className="mt-1">{t("advertising.filteredEmpty")}</p><div className="mt-3 flex flex-wrap gap-2"><a className="rounded-lg bg-blue-700 px-3 py-2 font-bold text-white" href="/templates/advertising-import-template.csv" download>{t("advertising.templateCta")}</a><a className="rounded-lg border border-blue-200 bg-white px-3 py-2 font-bold text-blue-800" href="/settings/import">{t("advertising.openImportCenter")}</a></div></div>
        <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm text-emerald-950"><p className="font-bold">ROAS</p><p className="mt-1">{t("advertising.roasExplanation")}</p></div>
        <div className="rounded-xl border border-purple-100 bg-purple-50 p-3 text-sm text-purple-950"><p className="font-bold">CPA / CPL</p><p className="mt-1">{t("advertising.cpaExplanation")} {t("advertising.cplExplanation")}</p></div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700"><p className="font-bold">{t("advertising.source")}: {t("advertising.manualSource")}</p><p className="mt-1">{t("advertising.attributionOptional")}</p><p className="mt-1">{t("advertising.manualAttributionRules")}</p></div>
      </section>
      <CampaignInsightsPanel rows={performance.data ?? []} campaigns={campaigns.data ?? []} currencyCode={currencyCode} />
      <section className="grid w-full max-w-full min-w-0 gap-3 overflow-hidden rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-sm text-indigo-950 shadow-sm" data-manual-attribution-summary="manual-campaign-id-orders-leads-meta-future"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-indigo-600">{t("advertising.manualAttributionEyebrow")}</p><h2 className="mt-1 text-xl font-black">{t("advertising.manualAttributionTitle")}</h2></div><div className="grid gap-2 md:grid-cols-2"><p>{t("advertising.manualAttributionManual")}</p><p>{t("advertising.manualAttributionLinkedOnly")}</p><p>{t("advertising.manualAttributionUnattributedValid")}</p><p>{t("advertising.manualAttributionMetaFuture")}</p></div></section>
      ...
      <section className="grid w-full max-w-full min-w-0 gap-4 overflow-hidden rounded-2xl bg-white p-4 shadow-sm" data-advertising-readiness-gate="not-pilot-ready-meta-future-runtime-qa-pending"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">{t("advertising.readinessEyebrow")}</p><h2 className="mt-1 text-xl font-black text-slate-950">{t("advertising.readinessTitle")}</h2><p className="mt-1 text-sm text-slate-600">{t("advertising.readinessDescription")}</p></div><div className="grid gap-3 lg:grid-cols-3"><StatusList title={t("advertising.readinessReadyTitle")} items={readinessReady} tone="ready" /><StatusList title={t("advertising.readinessPendingTitle")} items={readinessPending} tone="pending" /><StatusList title={t("advertising.readinessMetaTitle")} items={[t("advertising.readinessMetaFuture")]} tone="future" /></div></section>

      {isCampaignCreateOpen ? <FormDialog title={t("advertising.createCampaign")} description="Налаштуйте кампанію, бюджет, період і ціль реклами." size="lg" onClose={() => setIsCampaignCreateOpen(false)}><CampaignForm onSubmit={(payload) => createCampaignMutation.mutate(payload)} /></FormDialog> : null}
      {isMetricCreateOpen ? <FormDialog title={t("advertising.addDailyMetric")} description="Внесіть витрати та результати за один день." size="lg" onClose={() => setIsMetricCreateOpen(false)}><AdMetricForm campaigns={campaigns.data ?? []} onSubmit={(payload) => createMetricMutation.mutate(payload)} /></FormDialog> : null}
      {archivingCampaign ? <ConfirmActionDialog title="Архівувати кампанію?" description="Кампанію буде приховано з активних списків. Історичні метрики залишаться збереженими." actionLabel="Архівувати кампанію" isSubmitting={archiveCampaignMutation.isPending} error={archiveCampaignMutation.isError ? safeApiErrorMessage(archiveCampaignMutation.error, "Не вдалося архівувати кампанію. Спробуйте ще раз.") : null} onCancel={() => setArchivingCampaign(null)} onConfirm={() => archiveCampaignMutation.mutate()} /> : null}
      {deletingMetric ? <ConfirmActionDialog title="Видалити метрику?" description="Денну метрику буде видалено з підсумків, трендів і розрахунків ефективності." actionLabel="Видалити метрику" isSubmitting={deleteMetricMutation.isPending} error={deleteMetricMutation.isError ? safeApiErrorMessage(deleteMetricMutation.error, "Не вдалося видалити метрику. Спробуйте ще раз.") : null} onCancel={() => setDeletingMetric(null)} onConfirm={() => deleteMetricMutation.mutate()} /> : null}
      {editingCampaign ? <EditRecordDialog title={t("advertising.editCampaign")} fields={[{ name: "name", label: t("advertising.campaignName") }, { name: "platform", label: t("advertising.platform"), type: "select", options: PLATFORM_OPTIONS }, { name: "status", label: t("tables.status"), type: "select", options: STATUS_OPTIONS }, { name: "objective", label: t("advertising.objective"), type: "select", options: OBJECTIVE_OPTIONS }, { name: "budget_type", label: "Тип бюджету", type: "select", options: BUDGET_OPTIONS }, { name: "daily_budget", label: t("advertising.dailyBudget"), type: "number" }, { name: "total_budget", label: t("advertising.totalBudget"), type: "number" }, { name: "start_date", label: "Дата початку", type: "date" }, { name: "end_date", label: "Дата завершення", type: "date" }, { name: "notes", label: t("advertising.notes"), type: "textarea" }]} initialValues={editingCampaign} isSubmitting={updateCampaignMutation.isPending} submitError={updateCampaignMutation.isError ? safeApiErrorMessage(updateCampaignMutation.error, "Не вдалося зберегти зміни кампанії.") : null} onClose={() => setEditingCampaign(null)} onSubmit={(values) => updateCampaignMutation.mutate(values)} /> : null}
      {editingMetric ? <EditRecordDialog title={t("advertising.editMetric")} fields={[{ name: "metric_date", label: "Дата метрики", type: "date" }, { name: "spend", label: t("advertising.spend"), type: "number" }, { name: "impressions", label: "Покази", type: "number" }, { name: "reach", label: "Охоплення", type: "number" }, { name: "clicks", label: "Кліки", type: "number" }, { name: "messages", label: t("advertising.messages"), type: "number" }, { name: "leads", label: t("advertising.leads"), type: "number" }, { name: "orders", label: t("advertising.orders"), type: "number" }, { name: "revenue", label: t("advertising.revenue"), type: "number" }, { name: "net_profit", label: t("finance.netProfit"), type: "number" }]} initialValues={editingMetric} isSubmitting={updateMetricMutation.isPending} submitError={updateMetricMutation.isError ? safeApiErrorMessage(updateMetricMutation.error, "Не вдалося зберегти зміни метрики.") : null} onClose={() => setEditingMetric(null)} onSubmit={(values) => updateMetricMutation.mutate(values)} /> : null}
    </WorkspacePage>
  );
}
