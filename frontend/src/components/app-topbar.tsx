    ...
    <header className="mobile-safe-top sticky top-0 z-50 min-w-0 border-b border-border-subtle bg-canvas/92 px-3 py-3 text-text-primary backdrop-blur-xl sm:px-4 md:px-6 lg:static lg:h-[var(--topbar-height)] lg:bg-canvas/92 lg:py-0" data-shell-topbar>
      <div className="mobile-topbar-compact flex w-full min-w-0 items-center gap-2 sm:gap-3 lg:h-full lg:gap-4">
        ...
        <div className="relative hidden md:block">
          <button
            className="relative grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-border-subtle bg-surface-2 text-text-secondary shadow-sm transition hover:border-border-strong hover:bg-surface-hover hover:text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring"
            aria-label={t("topbar.notifications")}
            aria-expanded={notificationsOpen}
            onClick={() => setNotificationsOpen((value) => !value)}
            data-direct-notification-bell
          >
            <Bell className="h-5 w-5" />
            {unreadTotal > 0 ? (
              <span className="absolute -right-1.5 -top-1.5 grid min-h-5 min-w-5 place-items-center rounded-full bg-rose-500 px-1 text-[0.65rem] font-black text-white">
          ...
          {notificationsOpen ? (
            <div className="absolute right-0 top-12 z-[70] w-[min(92vw,420px)] overflow-hidden rounded-3xl border border-border-subtle bg-surface-1 shadow-2xl" data-direct-notification-center>
              <div className="flex items-start justify-between gap-3 border-b border-border-subtle p-4">
                <div>
                  <p className="font-black">Сповіщення Direct</p>
                  <p className={`mt-1 text-xs font-semibold ${isError ? "text-rose-600" : isLive ? "text-emerald-600" : "text-amber-600"}`}>
                    {isError ? "Live-зʼєднання тимчасово недоступне" : isLive ? "Live-оновлення активні" : "Підключення до live-оновлень…"}
                  </p>
                </div>
              ...
              {permission === "default" ? (
                <div className="border-b border-border-subtle p-3">
                  <button type="button" onClick={() => void requestBrowserNotifications()} className="min-h-10 w-full rounded-2xl bg-primary/15 px-3 text-sm font-black text-primary hover:bg-primary/20">
                    Увімкнути системні сповіщення
                  </button>
                </div>
              ) : null}
              <div className="max-h-[420px] overflow-y-auto p-2">
                {(summary?.events ?? []).length === 0 ? (
                  <p className="p-4 text-center text-sm text-text-muted">Нових непрочитаних повідомлень немає.</p>
                ) : (
                  (summary?.events ?? []).slice(0, 10).map((event) => (
                    <button key={event.message_id} type="button" onClick={() => openNotification(event)} className="flex w-full items-start gap-3 rounded-2xl p-3 text-left transition hover:bg-surface-hover">
                      <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ${event.order_intent ? "bg-amber-500/15 text-amber-600" : "bg-primary/15 text-primary"}`}>
                        {event.order_intent ? <ShoppingBag className="h-4 w-4" /> : <Bell className="h-4 w-4" />}
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center justify-between gap-2">
                          <span className="truncate text-sm font-black">{participantLabel(event)}</span>
                          {event.order_intent ? <span className="shrink-0 rounded-full bg-amber-500/15 px-2 py-0.5 text-[0.65rem] font-black text-amber-700">Ймовірне замовлення</span> : null}
                        </span>
                        <span className="mt-1 block line-clamp-2 text-xs text-text-muted">{event.text_preview}</span>
                      </span>
                    </button>
                  ))
                )}
