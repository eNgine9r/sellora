...
export function AppSidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col bg-[#080812] text-white">
      <div className="px-5 py-5">
        <Link href="/dashboard" onClick={onNavigate} aria-label="Sellora dashboard">
          <BrandLogo className="h-auto w-44 brightness-0 invert" />
        </Link>
      </div>
      <nav className="grid gap-1 overflow-y-auto px-3 pb-5" aria-label="Main navigation">
        {items.map(([href, label, Icon]) => {
          ...
          return (
            <Link
              ...
              className={`group flex min-h-11 items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition ${
