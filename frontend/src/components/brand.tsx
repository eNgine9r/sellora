...
export function BrandLockup({ className = "", markClassName = "h-10 w-10", textClassName = "text-white" }: { className?: string; markClassName?: string; textClassName?: string }) {
  return (
    <span className={`inline-flex min-w-0 items-center gap-3 ${className}`}>
      <span className="grid shrink-0 place-items-center overflow-hidden rounded-2xl bg-white/5 p-1 ring-1 ring-white/10"><BrandIcon priority className={markClassName} /></span>
